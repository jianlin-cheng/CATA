	...
	while (iteration < n_epochs):
		...
		for j in range(parameters.m_num_items):
			...
			if m>0 :
				...
				x = a * np.sum(parameters.m_U[user_ids, :], axis=0) + parameters.lambda_v * parameters.m_theta[j,:] 
				parameters.m_V[j, :] = scipy.linalg.solve(A, x)
			else:
				# m=0, this article has never been rated
				A = np.copy(vv)
				x = parameters.lambda_v * parameters.m_theta[j,:]
				parameters.m_V[j, :] = scipy.linalg.solve(A, x)
		iteration += 1
...
def CAttAE(X, data, sparse, data_name, output_name, pretrain):
    ...
    # Take the average performance of thge three splits
